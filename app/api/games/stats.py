...
async def get_game_stats(game_id: int, db: AsyncSession = Depends(get_db)):
    ...
    for ts in team_stats:
        team_stats_response.append(GameStatsTeamEntry(
            ...
            saves=ts.saves,
            minutes=ts.minutes,
            xg=to_finite_float(ts.xg),
            freekicks=ts.freekicks,
            freekick_shots=ts.freekick_shots,
            freekick_passes=ts.freekick_passes,
            throw_ins=ts.throw_ins,
            goal_kicks=ts.goal_kicks,
            assists=ts.assists,
            passes_forward=ts.passes_forward,
            passes_progressive=ts.passes_progressive,
            key_passes=ts.key_passes,
            passes_to_final_third=ts.passes_to_final_third,
            passes_to_box=ts.passes_to_box,
            crosses=ts.crosses,
            shot_accuracy=to_finite_float(ts.shot_accuracy),
            corner_accuracy=to_finite_float(ts.corner_accuracy),
            freekick_shot_accuracy=to_finite_float(ts.freekick_shot_accuracy),
            freekick_pass_accuracy=to_finite_float(ts.freekick_pass_accuracy),
            throw_in_accuracy=to_finite_float(ts.throw_in_accuracy),
            goal_kick_accuracy=to_finite_float(ts.goal_kick_accuracy),
            penalty_accuracy=to_finite_float(ts.penalty_accuracy),

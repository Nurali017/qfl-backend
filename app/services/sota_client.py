...
class SotaClient:
    ...
    async def get_live_match_stats(self, game_id: str) -> list[dict[str, Any]]:
        """Get live match statistics from /em/{game_id}-stat.json endpoint."""
        await self.ensure_authenticated()
        url = f"https://sota.id/em/{game_id}-stat.json"
        response = await self._make_request(
            "get",
            url,
            params={"access_token": self.access_token},
